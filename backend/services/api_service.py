...
class ApiService:
    ...
    async def _search_audible(self, query: str) -> list:
        ...
        try:
            ...
            response = requests.get(self.audible_api_url, params=params, headers=headers, timeout=10)
            ...
            if response.status_code == 200:
                ...
                for author in data.get('authors', [])[:3]:
                    if isinstance(author, dict) and author.get('author'):
                        ...
                        if author_id:
                            try:
                                author_resp = requests.get(f"https://openlibrary.org{author_id}.json", timeout=3)
