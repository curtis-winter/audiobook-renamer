...
1. **Select file(s)** from the file list on the left - click checkboxes to select multiple files for combining, or click a file to edit its metadata
...
5. **Apply Changes** - green button at top of metadata section
   - If multiple files selected: button changes to "Combine X Files and Apply Changes"
6. **Template editing** - click the ⚙ cog icon next to "File Name" or "File Location" to edit templates

### Multi-File Selection

- Click the checkbox next to any file to add it to the selection
- When 2+ files are selected:
  - "Combine X Files and Apply Changes" button appears in the metadata section
  - Sort controls appear in the file list
- Click a file row to view its metadata (checkbox still toggles selection)
- Clear selection by clicking the X on the metadata section or selecting files individually
...
- Click the **«** button (in a circle) on the divider to collapse the sidebar
- Click the **»** button to expand
- The collapse/expand button is always visible regardless of sidebar state
- When collapsed, the file list is hidden but you can still expand it

### Progress Feedback

During long operations (combining, moving, renaming, writing metadata), a modal appears showing:
- "Combining files..." - when combining multiple audio files
- "Moving files..." and "Writing metadata..." - when applying changes
...
| `/api/apply` | POST | Apply metadata changes to single file |
| `/api/combine` | POST | Combine multiple files and apply metadata |
| `/api/watch/status` | GET | Get watch status |
| `/api/watch/start` | POST | Start watching folder |
| `/api/watch/stop` | POST | Stop watching folder |
| `/api/delete-folder` | POST | Delete an empty folder |
